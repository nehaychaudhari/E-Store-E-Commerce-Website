...
onAuthStateChanged(auth, (user) => {
  if (user) {
    const uid = user.uid;
    const sellerProfileRef = ref(db, `seller/${uid}/profile`);
    ...
    get(sellerProfileRef).then((snapshot) => {
      if (snapshot.exists()) {
    ...
    profileForm.addEventListener("submit", async (e) => {
      ...
      if (profilePicInput.files.length > 0) {
        ...
        // If already present, keep existing image
        const img = document.getElementById("profile-pic-preview");
        if (img && img.src) {
          photoURL = img.src;
        }
      ...
      await update(sellerProfileRef, data);
